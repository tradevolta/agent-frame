...
function layout(title: string, body: string, cta?: { label: string; href: string }): string {
  return `<!doctype html><html><body style="margin:0;background:#f6f7f9;font-family:Helvetica,Arial,sans-serif;color:#14161a">
...
${body}
${cta ? `<p style="margin:24px 0 0"><a href="${cta.href}" style="background:#2446a6;color:#fdfdfe;text-decoration:none;padding:12px 20px;border-radius:8px;display:inline-block;font-weight:600">${esc(cta.label)}</a></p>` : ""}
</div>
<p style="font-size:12px;color:#5b6270;margin-top:20px">Questions? Just reply or write ${esc(brand.supportEmail)}. Keep this email. The link above is your private studio link.</p>
</div></body></html>`;
}
...
<p style="background:#f6f7f9;padding:12px;border-radius:8px;word-break:break-all"><b>${appUrl(`/join/${joinCode}`)}</b></p>
