...
export default function CitiesIndex() {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-14">
      <h1 className="font-display text-4xl font-semibold">Realtor headshots by city</h1>
      {Object.entries(byState).map(([state, cities]) => (
        <section key={state} className="mt-8">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
            {cities!.map((c) => (
              <Link key={c.slug} href={`/realtor-headshots/${c.slug}`} className="card p-3 text-sm hover:border-navy">
