    ...
    <div className="grid gap-8 lg:grid-cols-[1fr_320px]">
      <div className="space-y-8">
        <section className="card p-6">
          ...
          <div
            className="mt-4 grid cursor-pointer place-items-center rounded-xl border-2 border-dashed border-line bg-paper p-8 text-center hover:border-navy/40"
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            <span className={enough ? "font-medium text-ok" : "text-muted"}>
              {uploads.length} uploaded{pending ? `, ${pending} uploading…` : ""} {enough ? "✓" : `(need ${props.minUploads - uploads.length} more)`}
            </span>
          </div>
          {uploads.length > 0 && (
            <div className="mt-3 grid grid-cols-4 gap-2 sm:grid-cols-6 md:grid-cols-8">
              {uploads.map((u) => (
                <div key={u.id} className="group relative">
                  ...
                  <button onClick={() => remove(u.id)} className="absolute right-1 top-1 hidden h-6 w-6 rounded-full bg-black/70 text-xs text-white group-hover:block" aria-label="Remove photo">
                    ✕
                  </button>
                </div>
              ))}
            ...
            <div>
              ...
              <div className="flex gap-2">
                {(["woman", "man", "person"] as const).map((s) => (
                  <button key={s} onClick={() => setSubject(s)} className={`btn-ghost flex-1 !px-2 capitalize ${subject === s ? "!border-navy !bg-navy/5" : ""}`}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            {!props.isTeam && (
              <div className="sm:col-span-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {props.colors.map((c) => (
                    <button key={c.id} onClick={() => setColor(c.id)} className={`flex items-center gap-2 rounded-lg border px-3 py-2 text-sm ${color === c.id ? "border-navy" : "border-line"}`}>
        ...
        <section className="card p-6">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3">
            {props.styles.map((s) => {
              ...
              return (
                <button key={s.id} onClick={() => toggle(s.id)} className={`rounded-lg border p-3 text-left text-sm ${on ? "border-navy bg-navy/5" : "border-line bg-card"} ${pickAll ? "cursor-default" : ""}`}>
                  <div className="font-medium">{on ? "✓ " : ""}{s.name}</div>
      ...
      <aside className="h-fit lg:sticky lg:top-24">
        <div className="card p-6">
          ...
          <ul className="mt-3 space-y-1 text-sm">
            <li className={enough ? "text-ok" : "text-muted"}>{enough ? "✓" : "○"} {props.minUploads}+ photos</li>
            <li className={subject ? "text-ok" : "text-muted"}>{subject ? "✓" : "○"} About you</li>
            <li className={stylesOk ? "text-ok" : "text-muted"}>{stylesOk ? "✓" : "○"} Styles</li>
          </ul>
          ...
          <button onClick={submit} disabled={!ready || submitting} className="btn-gold mt-4 w-full">
